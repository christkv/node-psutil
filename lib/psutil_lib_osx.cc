...
v8::Handle<v8::Value> PSUtilLib::VirtualMemory(const v8::Arguments& args) {
  v8::HandleScope scope;
  ...
  // If we have a single parameter
  callback = args.Length() == 1 ? v8::Local<v8::Function>::Cast(args[0]) : v8::Local<v8::Function>::Cast(args[1]);
  // Create a worker object and map the information
