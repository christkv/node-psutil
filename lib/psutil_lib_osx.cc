...
#include "workers/process_list_worker.h"
...
void PSUtilLib::Initialize(v8::Handle<v8::Object> target)
{
  ...
  NODE_SET_PROTOTYPE_METHOD(t, "process_list", PSUtilLib::ProcessList);
...
Handle<Value> PSUtilLib::ProcessList(const Arguments& args) {
  HandleScope scope;
  ...
  // Create a worker object and map the information
  ProcessListWorker *worker = new ProcessListWorker();
  worker->error = false;
  worker->request.data = worker;
  worker->callback = Persistent<Function>::New(callback);
