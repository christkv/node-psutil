...
PSUtil.prototype.cpu_percent = function cpu_percent(interval, perCpu, callback) {
  ...
  if(process.platform === 'linux') {
    ...
    library_function = self._lib.cpu_times;
  }

  // Retrieve the cpu times
  library_function(perCpu, function(err, cpu_times1) {
    if(err) callback(err);
    if(!Array.isArray(cpu_times1)) cpu_times1 = [cpu_times1];

    // Wait for the programmed interval
    setTimeout(function() {
      library_function(perCpu, function(err, cpu_times2) {
        if(err) callback(err);
...
PSUtil.prototype.pid_exists = function pid_exists(pid, callback) {
  ...
  if(process.platform === 'linux') {
    ...
    library_function = self._lib.pid_exists;
  }

  // Retrieve the cpu times
  library_function(pid, callback);
}
...
PSUtil.prototype.disk_partitions = function disk_partitions(all, callback) {
  ...
  if(process.platform === 'linux') {
    ...
    library_function = self._lib.disk_partitions;
  }

  // Retrieve the disk partitions
  library_function(function(err, partitions) {
...
PSUtil.prototype.disk_usage = function disk_usage(path, callback) {
  ...
  if(process.platform === 'linux') {
    ...
    library_function = self._lib.disk_usage;
  }

  library_function(path, callback);
}
...
var _cpu_times_linux = function _cpu_times_linux(self, perCpu, callback) {
  self._lib.sys_conf(0, function(err, result) {
    console.log("---------------------------------------------------- 1")
    console.dir(err)
    console.dir(result)
    callback(null, []);
    ...
    self._lib.virtual_memory(function(_err, _data) {
      ...
      for(var i = 0; i < lines.length; i++) {
        ...
        if(line.match(/^pswpin/)) {
          sin = parseInt(line.split(/ +/)[1] * 4 * 1024, 10);
        } else if(line.match(/^pswpout/)) {
          sout = parseInt(line.split(/ +/)[1] * 4 * 1024, 10);
        }
      }
    ...
    self._lib.virtual_memory(function(_err, _data) {
      ...
      for(var i = 0; i < lines.length; i++) {
        ...
        if(line.match(/^Cached:/)) {
          cached = parseInt(line.split(/ +/)[1] * 1024, 10);
        } else if(line.match(/^Active:/)) {
          active = parseInt(line.split(/ +/)[1] * 1024, 10);
        } else if(line.match(/^Inactive:/)) {
          inactive = parseInt(line.split(/ +/)[1] * 1024, 10);
        }
      }
