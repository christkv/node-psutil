...
PSUtil.prototype.cpu_percent = function cpu_percent(interval, perCpu, callback) {
  ...
  if(process.platform === 'linux') {
    ...
    self._lib.cpu_percent(interval, perCpu, function(err, cpu_times1) {
      if(err) callback(err);
      if(!Array.isArray(cpu_times1)) cpu_times1 = [cpu_times1];

      // Wait for the programmed interval
      setTimeout(function() {
        self._lib.cpu_percent(interval, perCpu, function(err, cpu_times2) {
          if(err) callback(err);
    ...
    // this._lib.cpu_percent(interval, perCpu, callback);
  }
}
