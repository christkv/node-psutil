  ...
  PSUtil._readFile('/proc/filesystems', 'ascii', function(err, filesystems) {
    ...
    self._lib.disk_partitions(function(err, partitions) {
      ...
      for(var j = 0; j < partitions.length; j++) {
        ...
        console.dir(partition.fstype)

        if(!all && physicalDevices.indexOf(partition.fstype)) continue;
        returnList.push(device);
      }
      console.log("---------------------------------------------------")
      // console.log(physicalDevices)
      // // console.dir(err)
      // console.dir(partitions.map(function(x) {return x.fstype}))
      console.dir(partitions)
